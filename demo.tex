...
\begin{frame}{Case study on a numerical example}
     \begin{columns}
       ...
       For case 4: \par
     $\small{w^T = [ 0.8847 \quad 0.0007]
 , b =-6.3180 } $\par $\small{\alpha  \approx 0\%,\beta  \approx 99.99 \% ,VA \approx}99.99 \% $
 For case 5: \par 
   $\small{w^T = [0.0481 \quad 5.2780]
 , b = -23.5837} $\par $\small{\alpha  \approx 3.3333\times 10^{-3} \%,\beta  \approx 99.97\% ,VA \approx}99.99 \% $
           \end{column}
        \begin{column}{0.5\textwidth}  %%<--- here
    \begin{figure}
        \centering
        \includegraphics[width=4cm]{fig/Cu3.eps}
        \caption{$y_n,y_a,y_m,y_{am}$ data(case 4)}
        \includegraphics[width=4cm]{fig/miniVarP.eps}
        \caption{ $y_n,y_a,y_{m},y_{am},y_{ms}$ data(case 5)}
        \end{figure}
